...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(
		description="CNN SHAP analysis on FFT tensor (labels: link|tap|fft)."
	)
	parser.add_argument(
		"--model-dir",
		type=Path,
...
def build_shap_tensor(
    ...
    starts = range(0, n_explain, chunk_size)

    for start in tqdm(
        starts,
        total=(n_explain + chunk_size - 1) // chunk_size,
        desc="Computing SHAP",
    ):
        ...
        if shap_values.shape != chunk.shape:
            raise ValueError(
                f"Expected SHAP chunk shape {chunk.shape}, got {shap_values.shape}"
            )
...
def _fft_idx_to_hz_label(
	...
	*,
	...
	# FFT features exclude DC, so positive-only fft0 corresponds to k=1.
	if not negative_freq:
		k = fft_idx + 1
	else:
		# Layout when negative_freq=True is [-freq_bins..-1, +1..+freq_bins].
...
def run() -> None:
	...
	print(
		f"Running FFT-SHAP with background_size={n_bg}, n_explain={n_explain}, "
		f"chunk_size={chunk_size}, shap_nsamples={shap_nsamples}"
	)
	print(f"FFT label mapping: fs={fs_hz:.4f}Hz, N={n_fft_time}, negative_freq={negative_freq}")
	...
	if args.scope == "all":
		title = "CNN FFT-SHAP Feature Importance - All Samples"
	elif args.scope == "misclassified":
		title = "CNN FFT-SHAP Feature Importance - Misclassified Samples"
	elif args.scope == "fp":
		title = "CNN FFT-SHAP Feature Importance - False Positives"
	elif args.scope == "fn":
		title = "CNN FFT-SHAP Feature Importance - False Negatives"
	else:
		title = f"CNN FFT-SHAP Feature Importance - {args.subclass} - Misclassified"
